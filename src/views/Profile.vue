...
<template>
  <div class="flex flex-col items-center ">
    <h1 class="text-center text-2xl font-normal leading-normal mt-0 mb-4 text-black">
      This is an example application to simulate relying party as <p class="text-blue-800">"Fachdienst"</p> to login into secure Pages in receiving access token after a successfully authentication!
    </h1>
    <div class="row grid grid-cols-3 divide-x" v-if="isLoggedIn">
      <div class="shadow-2xl text-center bg-indigo-100 py-2 px-4 text-black rounded-t-2xl">
        <h2 class="bg-gradient-to-r from-indigo-800 to-blue-800 text-center text-3xl font-semibold leading-normal  rounded-t-2xl -mt-2 mb-0 -ml-4 -mr-4 text-white ">
          Person Profile
        </h2>
      ...
      <div class="shadow-2xl text-center bg-indigo-100 py-2 px-4 text-black rounded-t-2xl">
        ...
        <div class="flex justify-center  py-2 px-4 ">
          <div v-for="(value, name) in claimsDisplay" v-bind:key="value">
            <div class="border mt-10" v-if="name == 'azp'" >
              <button :disabled="false" class="shadow-2xl bg-pink-900 hover:bg-green-600 text-white font-bold py-14 px-14 mb-2.5 mb-2.5 mr-2 rounded h-250 w-125 hover:scale-150"
                  @click="value">
                {{ value }}
              </button>
              <button :disabled="false" class="shadow-2xl bg-gray-500 hover:bg-green-600 text-white font-bold py-14 px-14 mb-2.5 mb-2.5 rounded h-250 w-125 hover:scale-150"
                      @click="value">
                Example Application
              </button>
            </div>
        ...
        <h2 class="bg-gradient-to-r from-indigo-800 to-blue-800 text-center text-3xl font-semibold leading-normal mt-10 mb-0 -ml-4 -mr-4 text-white ">
          Scope
        </h2>
        <div class="flex justify-center  py-2 px-4  ">
